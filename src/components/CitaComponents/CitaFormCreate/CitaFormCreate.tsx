...
const CitaFormCreate = (props: Props) => {
    ...
    const verificar =() =>{
      ...
      const fechaHora = document.getElementById('fechaHora') as HTMLButtonElement;
      const attribute = document.getElementById('attribute') as HTMLButtonElement;
      const motivo = document.getElementById('motivo') as HTMLButtonElement;


      if(fechaHora.value.length <LongMin || fechaHora.value.length >LongMax )
        bien = false
      
      if(attribute.value.length <LongMin || attribute.value.length >LongMax )
        bien = false

      if(motivo.value.length <LongMin || motivo.value.length >LongMax )
        bien = false
    ...
    <div>
      ...
      <form className='form-control' onSubmit={handleSubmit}>
        ...
        <div  className='form-floating'>
          <input id='attribute' name='attribute'  placeholder=''  className="form-control"   type="number" value={attribute11} required onChange={(e) => setAttribute11(e.target.value)} />
        ...
        <div className=''>
          ...
          <textarea  className="form-control col-6" minLength={LongMin} maxLength={LongMax} value={motivoCita} name="motivo" id="motivo" rows={5} placeholder="Motivos...." onChange={(e) => setMotivoCita(e.target.value)} required></textarea>
        </div>
