...
const PacienteFormUpdate = ({id}: Props) => {
  ...
  return (
    ...
    <form className='form-control' onSubmit={handleSubmit}>
      ...
      <div  className='form-floating'>
        <input name='nss'  placeholder=''  className="form-control"  min={100000000000} max={999999999999}  type="number" value={nss} required onChange={(e) => setNss(e.target.value)} />
        <label  htmlFor='nss' className="form-label">Número de la seguridad social:</label>
      </div>
      <div className='form-floating'>
        <input name='numTarjeta' maxLength={LongMax}   placeholder='' minLength={LongMin}  className="form-control" type="text" value={numTarjeta} required onChange={(e) => setNumTarjeta(e.target.value)} />
        <label  htmlFor='numTarjeta'  className="">Número de tarjeta:</label>
      </div>
      <div className='form-floating'>
        <input name='telefono' min={100000000} max={999999999} placeholder='' minLength={LongMin}  className="form-control" type="number" value={telefono} required onChange={(e) => setTelefono(e.target.value)} />
        <label  htmlFor='telefono'  className="">Teléfono:</label>
      </div>
      <div className='form-floating'>
        <input name='direccion' maxLength={LongMax} placeholder='' minLength={LongMin}  className="form-control" type="text" value={direccion} required onChange={(e) => setDireccion(e.target.value)} />
      ...
      <div className='row justify-content-around align-items-center mb-2 mt-3'>
        ...
        <Link className=" col-3 btn btn-warning " to={`/paciente/${id}`} >Volver </Link>
      </div>
